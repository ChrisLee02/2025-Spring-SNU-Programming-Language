...
let rec collect_conditions : map * env -> shape * condition list =
 fun (map, env) ->
  match map with
  | End StarBox ->
      if Hashtbl.mem env "*" then (SBar, [])
        ...
        (SBar, []))
  | End (NameBox name) ->
      if Hashtbl.mem env name then (Hashtbl.find env name, [])
      else
        let shape_var = new_shape_var () in
        Hashtbl.add env name shape_var;
        (shape_var, [])
  | Branch (map1, map2) ->
      let shape1, conds1 = collect_conditions (map1, env) in
      let shape2, conds2 = collect_conditions (map2, env) in
      let shape_var = new_shape_var () in
      let branch_condition = Equal (shape1, SNode (shape2, shape_var)) in
      (shape_var, conds1 @ conds2 @ [ branch_condition ])
  | Guide (name, map) ->
      let shape, conds = collect_conditions (map, env) in
      let shape_var = new_shape_var () in
      let guide_condition =
        Equal (shape_var, SNode (Hashtbl.find env name, shape))
      in
...
let rec apply_subst (subst : subst) (s : shape) : shape =
  ...
  | SVar n -> (
      ...
      | None -> SVar n)
  | SNode (s1, s2) -> SNode (apply_subst subst s1, apply_subst subst s2)

let rec resolve_conditions (conds : condition list) (subst : subst) : subst =
  match conds with
  | [] -> subst
  | Equal (s, t) :: rest -> (
      ...
      match (s', t') with
      | _ when s' = t' -> resolve_conditions rest subst
      | SVar n, _ when not (occurs n t') ->
          resolve_conditions rest ((n, t') :: subst)
      | _, SVar n when not (occurs n s') ->
          resolve_conditions rest ((n, s') :: subst)
      | SNode (s1, s2), SNode (t1, t2) ->
          resolve_conditions (Equal (s1, t1) :: Equal (s2, t2) :: rest) subst
      | _ -> raise IMPOSSIBLE)

let getReady : map -> key list =
 fun map ->
  let env : env = Hashtbl.create 10 in
  let root_shape, conds = collect_conditions (map, env) in
  let subst = resolve_conditions conds [] in
  let keys =
    Hashtbl.fold
      (fun _ s acc ->
