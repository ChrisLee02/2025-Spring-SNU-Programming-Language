...
module Env : ENV = struct
  ...
  let bind (E env) id loc =
    E (fun x -> if x = id then loc else env x)
end
...
module K : KMINUS = struct
  ...
  and env_entry =
    | Addr of Loc.t
    | Proc of id list * exp * env

  let emptyMemory = Mem.empty
  let emptyEnv = Env.empty

  let empty_record : id -> Loc.t =
   fun x -> raise EmptyRecord
  ...
  let rec eval mem env e =
    ...
    | LETV (x, e1, e2) ->
        ...
        eval
          (Mem.store mem'' l v)
          (Env.bind env x (Addr l))
          e2
    | LETF (f, args, fBody, e) ->
        (* Proc of id list * exp * env 로 env_entry를 만들어서 등록  *)
        eval mem
          (Env.bind env f (Proc (args, fBody, env)))
          e
    | CALLV (f, e_list) ->
        ...
        if List.length args != List.length e_list then
          ...
          let v_list, mem' =
            map_with_state eval_with_env mem e_list
          in

          let loc_list, mem'' =
            map_with_state save_value_mem mem' v_list
          in

          let fEnv_args_binded =
            List.fold_left2
              (fun env arg loc ->
                Env.bind env arg (Addr loc))
              fEnv args loc_list
          in

          let new_fEnv =
            Env.bind fEnv_args_binded f
              (Proc (args, fBody, fEnv))
          in
    ...
    | RECORD key_value_list ->
        ...
        *)
        if List.is_empty key_value_list then (Unit, mem)
          ...
          let e_list =
            List.map (fun (x, y) -> y) key_value_list
          in

          let id_list =
            List.map (fun (x, y) -> x) key_value_list
          in
          ...
          let record : id -> Loc.t =
            List.fold_left2
              (fun acc id loc ->
                fun key -> if key = id then loc else acc key)
    ...
    | IF (econd, et, ee) ->
        ...
        if cond_b1 then eval mem' env et
        else eval mem' env ee
    | WHILE (econd, et) ->
        let cond_val, mem' = eval mem env econd in
        let cond_b1 = value_bool cond_val in
