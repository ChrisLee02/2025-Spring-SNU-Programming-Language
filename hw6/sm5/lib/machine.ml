...
let loc_to_str (base, offset) = Printf.sprintf "<%d, %d>" base offset
...
and evalue_to_str indent ev =
  match ev with Loc l -> loc_to_str l | Proc p -> proc_to_str indent p
...
let stack_to_str s = "[" ^ String.concat ";\n" (List.map sval_to_str s) ^ "]"
...
let load l m =
  try List.assoc l m
  with Not_found ->
    raise (Error ("Uninitialized location : %s" ^ loc_to_str l))

let store l v m =
  if List.mem_assoc l m then (l, v) :: List.remove_assoc l m else (l, v) :: m
...
let is_equal v1 v2 =
  ...
  | Z z1, Z z2 -> z1 = z2
  | B b1, B b2 -> b1 = b2
  | R r1, R r2 -> List.sort compare r1 = List.sort compare r2
  | Unit, Unit -> true
...
let step : smeck -> smeck = function
  ...
  | s, m, e, MALLOC :: c, k ->
      ...
      else (V (L (malloc ())) :: s, m, e, c, k)
  | s, m, e, BOX z :: c, k ->
      if z <= 0 then raise (Error "Box(n): non-positive n")
  ...
  | V (L (base, offset)) :: V (Z z) :: s, m, e, ADD :: c, k ->
      ...
      else raise (Error "Negative loc offset spawned")
