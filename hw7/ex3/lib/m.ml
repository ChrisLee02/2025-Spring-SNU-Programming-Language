...
module M : sig
  ...
  let op2fn = function
    ...
    | EQ -> (
        fun (v1, v2) ->
          ...
          | _ -> raise (TypeError "type mismatch in ="))
  ...
  let rec eval env mem exp =
    ...
    | LET (VAL (id, e1), e2) ->
        let v1, m' = eval env mem e1 in
        eval (env @+ (id, v1)) m' e2
    | LET (REC (fun_id, arg_id, body), e) ->
        let rec_fun = RecFun (fun_id, arg_id, body) in
        let env' = env @+ (fun_id, Closure (rec_fun, env)) in
        eval env' mem e
    | MALLOC e ->
        let v, m' = eval env mem e in
        let l, m'' = malloc m' in
