...
let subst_scheme : subst -> typ_scheme -> typ_scheme =
 fun subs tyscm ->
  ...
  | GenTyp (alphas, t) ->
      ...
      let s' =
        List.fold_left2
          (fun acc alpha beta -> make_subst alpha (TVar beta) @@ acc)
...
let rec infer (tyenv : typ_env) (exp : M.exp) : typ * subst =
  ...
  | BOP (op, e1, e2) -> (
      ...
      | EQ -> (
          ...
          | TInt | TBool | TString | TLoc _ ->
              ...
              (TBool, s3 @@ s2 @@ s1)
          | _ ->
              raise (M.TypeError ("EQ 연산에 허용되지 않는 타입" ^ string_of_typ resolved))
          ))
  ...
  | WRITE e -> (
      ...
      | _ -> raise (M.TypeError ("WRITE 연산에 허용되지 않는 타입" ^ string_of_typ t')))
  | MALLOC e ->
      let t, s = infer tyenv e in
      (TLoc t, s)
  | ASSIGN (e1, e2) ->
